...
unsigned put_info(const std::vector<std::string>& inputs, std::string& request, std::string& response)
{
    std::cout << "Server receive: "  << request << std::endl;
    response = " Answer  = Yes ";
...
int main(int argc, char* argv[])
{
    ...
    auto ret = client.send_request(hp::http::Methods::GET, "/info", input_data, result, 2000); // blocking until 2000 ms
    std::cout << "get error code: " << ret << " and response: " << result << std::endl;

    ret = client.send_request(hp::http::Methods::GET, "/info", input_data, result); // blocking
    std::cout << "get error code: " << ret << " and response: " << result << std::endl;
